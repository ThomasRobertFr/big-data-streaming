...
stream.foreachRDD(rdd => {
  ...
  if (rdd.count() > 0) {
    ...
    println("Processing time: "+(System.currentTimeMillis() - tic)+" ms")
  }
})
...
class SentimentAnalysis (val data: Map[String, Int]) extends java.io.Serializable  {
  def doIt (s: String) = s.split(" ").map(s => data.getOrElse(s.toLowerCase, 0)).reduceLeft(_ + _)
}

val data:Map[String, Int] = sc.textFile("hdfs:///dictionnary.txt").collect().map(x => {
  val vals = x.split("\t")
  (vals(0), vals(1).toInt)
}).toMap

val analyser = new SentimentAnalysis(data);

val rdd = sc.parallelize(Array("meow", ":)", ":(", "pouet"))
rdd.map(analyser.doIt).collect()
